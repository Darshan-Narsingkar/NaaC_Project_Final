...
if (isset($_GET['accept_id'])) {
    $id = $_GET['accept_id'];
    $sql = "UPDATE extension_outreach SET status = '1' WHERE id = '$id'";
...
if (isset($_GET['reject_id'])) {
    $id = $_GET['reject_id'];
    $sql = "UPDATE extension_outreach SET status = '2' WHERE id = '$id'";
...
$sql_count = "SELECT COUNT(*) as total FROM extension_outreach $where_clause";
$result_count = $conn->query($sql_count);
$total_records = $result_count->fetch_assoc()['total'];
$total_pages = ceil($total_records / $records_per_page);

// Fetch records with pagination
$sql = "SELECT * FROM extension_outreach $where_clause LIMIT $records_per_page OFFSET $offset";
...
<head>
    ...
    <title>extension_outreach Records (IT Department)</title>
...
<h2>extension_outreach Records (IT Department)</h2>

<!-- Search Form -->
<form method="post" action="Extension/hExtension.php">
        ...
        <tr class="<?= $status_class; ?>">
            ...
            <td>
                <?php if ($row['status'] == '0' || $row['status'] == '2') { ?>
                    <a href="Extension/hExtension.php?accept_id=<?= $row['id']; ?>" class="accept-btn">Accept</a>
                <?php } ?>

                <?php if ($row['status'] == '0' || $row['status'] == '1') { ?>
                    <a href="Extension/hExtension.php?reject_id=<?= $row['id']; ?>" class="reject-btn">Reject</a>
                <?php } ?>

                <?php if ($row['status'] != '2') { ?>
                    <a href="Extension/updateextension_outreach.php?id=<?= $row['id']; ?>">
...
<!-- Action Buttons -->
<button onclick="window.open('Extension/hprint_extension_outreach.php', '_blank')" class="action-btn print-btn">Print</button>
<button class="btn btn-danger print-btn action-btn" onclick="window.open('Extension/extension_outreach_to_pdf.php', '_blank')">Export to PDF</button>
<button class="btn btn-danger print-btn action-btn" onclick="window.open('#', '_blank')">View Analysis</button>

<!-- Pagination -->
<div style="text-align:center; margin-top:20px;">
    <?php for ($i = 1; $i <= $total_pages; $i++) { ?>
        <a href="Extension/hExtension.php?page=<?= $i; ?>" style="margin: 5px;"><?= $i; ?></a>
    <?php } ?>
