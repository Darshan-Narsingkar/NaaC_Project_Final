...
$query = "SELECT department, status, COUNT(*) as count FROM book_chapter_publications WHERE department='IT' GROUP BY department, status";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $data[$department]['counts'][] = (int)$row['count'];
}
...
<body>
    <h2>Book Chapter Publications Analysis (IT Department)</h2>
        ...
        ?>
            ...
            new Chart(ctxBar, {
                ...
                data: {
                    labels: labels,
                    datasets: [{
                        label: '<?php echo htmlspecialchars($department); ?> Book Chapter Publications',
                        data: counts,
