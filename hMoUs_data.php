...
if (isset($_GET['accept_id'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Record accepted successfully'); window.location.href='hod.php';</script>";
    } else {
...
if (isset($_GET['reject_id'])) {
    ...
    $sql = "UPDATE MoUs_data SET status = $status WHERE id = $id";
    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Record rejected successfully'); window.location.href='hod.php';</script>";
    } else {
        echo "<script>alert('Error updating record: " . addslashes($conn->error) . "');</script>";
    }
}
...
<head>
    ...
    <title>MoUs Records (IT Department)</title>
...
<h2>MoUs Records (IT Department)</h2>

<!-- Search Form -->
<form method="post" action="hMoUs_data.php">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                if ($row['status'] == 0 || $row['status'] == 2) {
                    echo "<a href='hMoUs_data.php?accept_id={$row['id']}' class='action-btn accept-btn'>Accept</a>";
                }
                if ($row['status'] == 0 || $row['status'] == 1) {
                    echo "<a href='hMoUs_data.php?reject_id={$row['id']}' class='action-btn reject-btn'>Reject</a>";
                }
                if ($row['status'] != 2): ?>
...
<button onclick="window.open('print__mous.php', '_blank')" class="action-btn print-btn">Print</button>
<button class="btn btn-danger print-btn action-btn" onclick="window.open('export_to_pdf.php', '_blank')">Export to PDF</button>

<button class="btn btn-danger print-btn action-btn" onclick="window.open('ITanalysis.php', '_blank')">View Analysis</button>
