...
if (isset($_GET['accept_id'])) {
    $id = $_GET['accept_id'];
    $sql = "UPDATE industry_academia SET status = '1' WHERE id = '$id'";
...
if (isset($_GET['reject_id'])) {
    $id = $_GET['reject_id'];
    $sql = "UPDATE industry_academia SET status = '2' WHERE id = '$id'";
...
$sql_count = "SELECT COUNT(*) as total FROM industry_academia $where_clause";
$result_count = $conn->query($sql_count);
$total_records = $result_count->fetch_assoc()['total'];
$total_pages = ceil($total_records / $records_per_page);

// Fetch records with pagination
$sql = "SELECT * FROM industry_academia $where_clause LIMIT $records_per_page OFFSET $offset";
...
<head>
    ...
    <title>industry_academia Records (IT Department)</title>
...
<h2>industry_academia Records (IT Department)</h2>

<!-- Search Form -->
<form method="post" action="Industry-Academia-Linkages/hIndustry_Academia_Linkages.php">
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $proof_file = !empty($row['proof_file']) ? "<a href='Entrepreneurship/files/{$row['proof_file']}' download>Download</a>" : "No file";
        ?>
        ...
        <tr class="<?= $status_class; ?>">
            ...
            <td>
                <?php if ($row['status'] == '0' || $row['status'] == '2') { ?>
                    <a href="Industry-Academia-Linkages/hIndustry_Academia_Linkages.php?accept_id=<?= $row['id']; ?>" class="accept-btn">Accept</a>
                <?php } ?>

                <?php if ($row['status'] == '0' || $row['status'] == '1') { ?>
                    <a href="Industry-Academia-Linkages/hIndustry_Academia_Linkages.php?reject_id=<?= $row['id']; ?>" class="reject-btn">Reject</a>
                <?php } ?>

                <?php if ($row['status'] != '2') { ?>
                    <a href="Industry-Academia-Linkages/updateindustry_academia.php"?id=<?= $row['id']; ?>">
...
<!-- Action Buttons -->
<button onclick="window.open('Industry-Academia-Linkages/hprint_industry_academia.php', '_blank')" class="action-btn print-btn">Print</button>
<button class="btn btn-danger print-btn action-btn" onclick="window.open('Industry-Academia-Linkages/industry_academia_to_pdf.php', '_blank')">Export to PDF</button>
<button class="btn btn-danger print-btn action-btn" onclick="window.open('#', '_blank')">View Analysis</button>

<!-- Pagination -->
<div style="text-align:center; margin-top:20px;">
    <?php for ($i = 1; $i <= $total_pages; $i++) { ?>
        <a href="Industry-Academia-Linkages/hIndustry_Academia_Linkages.php?page=<?= $i; ?>" style="margin: 5px;"><?= $i; ?></a>
    <?php } ?>
