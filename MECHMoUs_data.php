...
if (isset($_GET['accept_id'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Record accepted successfully');</script>";
    } else {
...
<head>
    ...
    <title>MoUs Records (MECH Department)</title>
...
<h2>MoUs Records (MECH Department)</h2>

<!-- Search Form -->
<form method="post" action="MECHMoUs_data.php">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                if ($row['status'] == 0 || $row['status'] == 2) {
                    echo "<a href='hMoUs_data.php?accept_id={$row['id']}' onclick=\"return confirm('Are you sure you want to accept this record?');\">
                              <button class='accept-btn'>Accept</button></a>";
                }

                if ($row['status'] == 0 || $row['status'] == 1) {
                    echo "<a href='hMoUs_data.php?reject_id={$row['id']}' onclick=\"return confirm('Are you sure you want to reject this record?');\">
...
<button onclick="window.open('MECHprint__mous.php', '_blank')" class="action-btn print-btn">Print</button>
<button class="btn btn-danger print-btn action-btn" onclick="window.open('MECHexport_to_pdf.php', '_blank')">Export to PDF</button>

<button class="btn btn-danger print-btn action-btn" onclick="window.open('MECHanalysis.php', '_blank')">View Analysis</button>
