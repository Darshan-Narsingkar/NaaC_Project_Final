...
if (isset($_GET['accept_id'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Record accepted successfully');window.location.href='EXTChod.php';</script>";
    } else {
...
if (isset($_GET['reject_id'])) {
    ...
    $sql = "UPDATE MoUs_data SET status = $status WHERE id = $id";
    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Record rejected successfully');window.location.href='EXTChod.php';</script>";
    } else {
        echo "<script>alert('Error updating record: " . addslashes($conn->error) . "');</script>";
    }
}
...
<head>
    ...
    <title>MoUs Records (EXTC Department)</title>
...
<h2>MoUs Records (EXTC Department)</h2>

<!-- Search Form -->
<form method="post" action="EXTCMoUs_data.php">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                if ($row['status'] == 0 || $row['status'] == 2) {
                    echo "<a href='EXTCMoUs_data.php?accept_id={$row['id']}' onclick=\"return confirm('Are you sure you want to accept this record?');\">
                              <button class='accept-btn'>Accept</button></a>";
                }

                if ($row['status'] == 0 || $row['status'] == 1) {
                    echo "<a href='EXTCMoUs_data.php?reject_id={$row['id']}' onclick=\"return confirm('Are you sure you want to reject this record?');\">
...
<button onclick="window.open('EXTCprint__mous.php', '_blank')" class="action-btn print-btn">Print</button>
<button class="btn btn-danger print-btn action-btn" onclick="window.open('EXTCexport_to_pdf.php', '_blank')">Export to PDF</button>

<button class="btn btn-danger print-btn action-btn" onclick="window.open('EXTCanalysis.php', '_blank')">View Analysis</button>
