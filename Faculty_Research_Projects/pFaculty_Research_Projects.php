...
if (!empty($selectedDepartment)) {
    ...
    $sql = "SELECT * FROM faculty_projects WHERE status='1' AND department = ?";
    ...
    $sql = "SELECT * FROM faculty_projects WHERE status='1'";
...
<div class="container">
    <h2 class="text-center mb-4">faculty_projects Records</h2>

    <!-- Department Dropdown -->
    <form method="GET" action="Faculty_Research_Projects/pFaculty_Research_Projects.php" class="mb-3">
    ...
    <?php else: ?>
        <div class="table-responsive">
                ...
                <tbody>
                    <?php foreach ($records as $record): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($record['pi_name']); ?></td>
                            <td><?php echo htmlspecialchars($record['co_investigators']); ?></td>
                            <td><?php echo htmlspecialchars($record['funding_agency']); ?></td>
                            <td><?php echo htmlspecialchars($record['grant_amount']); ?></td>
           ...
           <button class="btn btn-secondary" onclick="window.open('Faculty_Research_Projects/Print_principal.php', '_blank')">Print</button>
            <button class="btn btn-success" onclick="window.open('Faculty_Research_Projects/principalexport_to_pdf.php', '_blank')">Export to PDF</button>
            <button class="btn btn-danger print-btn action-btn" onclick="window.open('Faculty_Research_Projects/Analysis.php', '_blank')">View Analysis</button>
