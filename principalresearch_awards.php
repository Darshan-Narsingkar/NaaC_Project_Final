...
if (!empty($selectedDepartment)) {
    ...
    $sql = "SELECT * FROM  research_awards WHERE status='1' AND department = ?";
    ...
    $sql = "SELECT * FROM  research_awards WHERE status='1'";
...
<head>
    ...
    <title> research_awards - Principal Dashboard</title>
    <!-- Bootstrap CSS -->
...
<div class="container my-4">
    <h2 class="text-center mb-4">All  research_awards Signed During the Year</h2>

    <!-- Department Dropdown -->
    <form method="GET" action="principalresearch_awards.php" class="mb-3">
    ...
    <?php else: ?>
        <div class="table-responsive">
                ...
                <tbody>
                    <?php foreach ($records as $record): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($record['award_name']); ?></td>
                            <td><?php echo htmlspecialchars($record['faculty_name']); ?></td>
                            <td><?php echo htmlspecialchars($record['award_date']); ?></td>
                            <td><?php echo htmlspecialchars($record['award_organization']); ?></td>
           ...
           <button class="btn btn-secondary" onclick="window.open('analysis/Print_principal_award.php', '_blank')">Print</button>
            <button class="btn btn-success" onclick="window.open('analysis/awardexport_to_pdf.php', '_blank')">Export to PDF</button>
            <button class="btn btn-danger print-btn action-btn" onclick="window.open('analysis/awardAnalysis.php', '_blank')">View Analysis</button>
